...
<div class="max-w-6xl mx-auto my-4 py-8 px-6 bg-white shadow-md rounded-lg">
    ...
    <form action="{{ route('prepatelier.update', $preparation->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="border-l-4 border-green-600 pl-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="notes" id="notes" class="mt-2 block w-full border-gray-300 rounded-lg shadow-sm px-2 py-1" maxlength="255" rows="4">{{ old('notes', $preparation->notes) }}</textarea>
            </div>
            ...
            <div class="relative w-full">
                <div class="relative">
                    <input
                        ...
                        value="{{ old('employe', $preparation->employe ? $preparation->employe->prenom . ' ' . $preparation->employe->nom : '') }}"
                    >
                    <input type="hidden" id="employe_id" name="employe_id" value="{{ old('employe_id', $preparation->employe_id) }}">
        ...
        <div id="selected_employe" class="mb-6 p-4 bg-green-50 rounded-lg border border-green-200 hidden">
            ...
            <p id="selected_employe_info" class="text-green-700">{{ old('employe', $preparation->employe ? $preparation->employe->prenom . ' ' . $preparation->employe->nom : '') }}</p>
        </div>

        <!-- Partie Commande client -->
        <div class="border-l-4 border-green-600 pl-4">
            <h2 class="text-2xl font-bold text-gray-800 mb-4">Commande client</h2>
            <div class="mb-4">
            <label for="commande_id" class="block text-sm font-semibold text-gray-700">Commande client</label>
            <input type="text" id="commande_id" class="mt-2 block w-full border-gray-300 rounded-lg shadow-sm px-2 py-1 bg-gray-100 cursor-not-allowed" 
                   value="{{ $preparation->commande ? 'Commande n°' . $preparation->commande->id . ' : ' . $preparation->commande->client->nom . ' (' . $preparation->commande->client->code_client . ')' : 'Aucune commande sélectionnée' }}" 
                   readonly>
            <input type="hidden" name="commande_id" value="{{ $preparation->commande_id }}">
            </div>
        </div>
        ...
        <div class="border-l-4 border-green-600 pl-4">
            ...
            @foreach($preparation->etapes as $etape)
                ...
                <input type="text" name="etapes[]" value="{{ old('etapes', $etape->intitule) }}" class="mt-2 block w-full border-gray-300 rounded-lg shadow-sm px-2 py-1" placeholder="Description de l'étape" required>
                </div>
            @endforeach
            </div>
